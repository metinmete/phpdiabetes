...
require_once("include/labtest_variables.php");
...
$page_layouts["labtest_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	//	result - 
			...
			$format = $pSet->getViewFormat("result");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("result")))
				$class = ' rnr-field-number';
			$row["result_class"] = $class;
	//	unit - 
			$viewContainer->recId = $recordsCounter;
	...
	//	range - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("range", $data, $keylink);
			$row["range_value"] = $value;
			$format = $pSet->getViewFormat("range");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("range")))
				$class = ' rnr-field-number';
			$row["range_class"] = $class;
	//	comments - 
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("labtest", "detailspreview"));
