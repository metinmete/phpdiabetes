...
require_once("include/medication_variables.php");
...
$page_layouts["medication_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	//	medication - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("medication", $data, $keylink);
			$row["medication_value"] = $value;
			$format = $pSet->getViewFormat("medication");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("medication")))
				$class = ' rnr-field-number';
			$row["medication_class"] = $class;
	//	dose - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("dose", $data, $keylink);
			$row["dose_value"] = $value;
			$format = $pSet->getViewFormat("dose");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("dose")))
				$class = ' rnr-field-number';
			$row["dose_class"] = $class;
	//	no_pills - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("no_pills", $data, $keylink);
			$row["no_pills_value"] = $value;
			$format = $pSet->getViewFormat("no_pills");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("no_pills")))
				$class = ' rnr-field-number';
			$row["no_pills_class"] = $class;
	//	comments - 
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("medication", "detailspreview"));
