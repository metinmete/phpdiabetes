...
function checkTableName($shortTName, $type=false)
{
	...
	if ("readings" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("insulin" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("meal" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("exercise" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("medication" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	...
	if ("test_types" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("food_items" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("exercise_types" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("exam_type" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("inject_type" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("Readings_Chart" == $shortTName && ($type===false || ($type!==false && $type == 3)))
		return true;
	return false;
}
...
function GetUserPermissions($table="")
{
	...
	if($table=="inject_type")
	{
			$permissions =  "ADESPIM";
	}
	if($table=="Readings Chart")
	{
			$permissions =  "ADESPIM";
	}
...
function GetBaseScriptsForPage($isDisplayLoading, $additionalScripts = "", $customText = "")
{
	...
	$result .= "<script type=\"text/javascript\">var getCurrentLang=function(){return '".mlang_getcurrentlang()."';}</script>";
...
function xt_showchart($params)
{
	...
	if( !$useFlashChartLibrary && ( $params["ctype"]=="2DPie" || $params["ctype"]=="2DDoughnut" || $params["ctype"]=="Funnel" 
	...
	$chartParams = array();
	$chartParams['forceFlash'] = $useFlashChartLibrary;
	...
	if( !$params["singlePage"] )
	{
		...
		echo '<script type="text/javascript" language="javascript" src="' . GetRootPathForResources( 'libs/js/AnyChart.js' ) . '"></script>';
		echo '<script type="text/javascript" language="javascript" src="' . GetRootPathForResources( 'libs/js/AnyChartHTML5.js' ) . '"></script>';
