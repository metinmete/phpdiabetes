...
require_once("include/insulin_variables.php");
...
$page_layouts["insulin_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	//	doseage - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("doseage", $data, $keylink);
			$row["doseage_value"] = $value;
			$format = $pSet->getViewFormat("doseage");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("doseage")))
				$class = ' rnr-field-number';
			$row["doseage_class"] = $class;
	//	inject_type - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("inject_type", $data, $keylink);
			$row["inject_type_value"] = $value;
			$format = $pSet->getViewFormat("inject_type");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("inject_type")))
				$class = ' rnr-field-number';
			$row["inject_type_class"] = $class;
	//	comment - 
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("insulin", "detailspreview"));
